...
docker run --rm -v "$(pwd):/data" pandoc/latex --highlight-style=breezedark --pdf-engine=xelatex --from=markdown+tex_math_single_backslash+tex_math_dollars+raw_tex+raw_html+markdown_in_html_blocks -t beamer -o "$filename.pdf" -s $1
